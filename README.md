...
ファイル入出力はすべて rank 0 に閉じている (入力 `.ofd` は rank 0 が読んで
`comm_broadcast()` で配り、`ofd.log` / `ofd.out` / `time_series_data.h5` も
rank 0 だけが書く) ので、**共有ファイルシステムは不要**です。rank 0 を置く
ノードに入力があればよく、成果物もそこに集まります。

ノードを跨ぐと、スナップショットの集約が実ネットワークを流れます。
表示用の時系列が不要・粗くてよい場合は `hdf5` キーで抑えてください。

```
hdf5 = 0        # 時系列を出さない (集約の通信も行われない)
hdf5 = 1 500    # 間隔を粗くする (間引いたステップでは通信もしない)
```

集約は次の 2 点で軽くしてあります。

- 送るのはローカル配列全体ではなく rank 0 が実際に読む範囲だけ
  (ABC の余白を含まない。PML 8 層・100³/2 分割で 41% 削減)
- rank 0 は全域配列を持たず、届いた分をその場で出力バッファへ書き込む
  (逐次ストリーミング集約)

GPU をノードごとに割り当てるには `-hosts` を使います
(`cuda_mpi/rank2device.cu` がグローバル rank をノード内デバイス番号へ写像)。
ノード間の H 面交換はデバイス→ホストへコピーしてから MPI に渡すので、
**CUDA-aware MPI は不要**です。

```
mpiexec -n 8 ofd_cuda_mpi -gpu -hosts 4 2 2 2 2 -p 2 2 2 model.ofd
```

CI ではノードを跨いだ実行そのものは行えませんが、`--mca btl tcp,self` で
共有メモリ転送を無効化し、**物理的に別ノードへ送るときと同じ転送経路**を
単一ランナーで通す検証を入れています
(`data/sample/mpi_network_check.sh`、`build-mpi` / `build-cuda` で実行)。

```sh
sh data/sample/mpi_network_check.sh /path/to/bin/ofd_mpi /tmp/mpi-network
```

異機種のノードを混ぜた場合、丸め順序の違いでビット一致は保証されません
(物理的な正しさは保たれます)。
...
`ofd.log` と HDF5 の `/loss/P_loss` に書き出します。
**温度分布そのものは計算しません** (理由は下の制約を参照)。
...
- **CPU 版 (`ofd`) のみ**。`ofd_mpi` / `ofd_cuda` / `ofd_cuda_mpi` には
  このレイヤ自体が存在しません
- **P_loss は絶対値 (W/m³) ではなく入射振幅 1 あたりの相対量**です。
  実際の W/m³ にするには入射電力からの換算が要ります
- **温度分布は計算しません**。以前は 3 次元熱拡散
  (`updateTemperature`) を毎ステップ回していましたが、結果はどこにも
  出力されず捨てられていた上に、数値的にも物理的にも意味を持って
  いなかったため削除しました:
  - 時間刻みが FDTD の Dt (光学解析では ~1e-16 s) のままで、
    `T += Dt * P_loss` の増分 (~7e-18) が初期温度 20 の倍精度分解能
    (4.4e-15) に完全に吸収されていた
    (`thermal_slab.ofd` でも最後まで全セル T = 20 のまま)
  - 温度を周波数ごとに持っていた (温度は全周波数の発熱の合計で決まる
    単一のスカラー場)
  - ラプラシアンが一様格子 (平均 `(Xn[Nx]-Xn[0])/Nx` 等) 前提で、
    OpenFDTD が許す不等間隔メッシュに対応していなかった

  温度分布を出すには、熱拡散用の別の時間刻み (fs ではなく us〜s の
  オーダー)、入射電力からの絶対値換算、材料ごとの熱拡散係数、
  不等間隔格子のラプラシアンが要ります。ここで出力する P_loss が
  その入力になります
