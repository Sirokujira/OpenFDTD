...
void solve(int io, double *tdft, FILE *fp)
{
    ...
    // HDF5 の書き込みは rank 0 のみが行う (以降の H5Gcreate / H5Dcreate は
    // すべて if (io) の中にある)。並列 HDF5 (H5Pset_fapl_mpio) で開くと
    // これらのメタデータ操作が集団操作となり、rank 0 だけが呼ぶため
    // 全ランクが呼ぶ H5Fclose でメタデータ不整合となりデッドロックしていた。
    // 出力内容は従来どおり rank 0 の担当領域のみなので、逐次アクセスで開く。
            ...
            if (io) {
                ...
                for (int ifreq = 0; ifreq < NFreq2; ifreq++) {
                    ...
                    for (int nn = 0; nn < NN; nn++) {
                        ...
                        // H5Pset_dxpl_mpio は逐次オープンのファイルには使えないため削除 (rank 0 のみ書き込み)
