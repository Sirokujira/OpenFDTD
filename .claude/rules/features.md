...
近傍界 DFT は入射スペクトルで正規化される — `sol/setupDft.c` が波源波形を
DFT した `cFdft` で割る。**この正規化係数が 0 になると DFT が 0 除算で
NaN になり、散乱断面積などが静かに 0 になる**。ログは "normal end" で
終わるのでスモークテストでは検出できない。

実際に踏んだ: `setup_planewave()` のパルス幅パラメータ `Planewave.ai` が
`4 / (1.27e-4 / f0)` と 10^4 倍大きく、パルスが Dt より遥かに細くなって
入射波が全サンプル点で 0 になっていた。正しくは `4 / (1.27 / f0)`
(`sol/setup.c` が給電波形用に決める `Tw = 1.27 / f0` と同じ定義)。
**全周波数帯の平面波サンプルが断面積 0 のまま長期間放置されていた**。

- 現在は `cFdft` が 0 なら `setupDft()` がエラーで停止する (静かに壊れない)。
- `data/sample/sphere_rcs_check.sh` が完全導体球の Mie 厳密解と比較する。
  平面波の波源設定に手を入れたら必ず通すこと。
- 波源のスペクトルが `frequency2` に乗らない構成では `waveamp` で CW に
  切り替えられる (`tpa_slab.ofd` / `thermal_slab.ofd` がそうしている)。
