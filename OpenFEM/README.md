...
このディレクトリは独立したリポジトリになるよう自己完結しています
(`CMakeLists.txt` / `README.md` / `LICENSE` / `.gitignore` /
`.github/workflows/ci.yml` を自前で持ちます)。現在は OpenFDTD リポジトリの
`OpenFEM/` 以下に置かれているため、**CI は動きません**
(GitHub Actions はリポジトリ直下の `.github/workflows/` しか読まないため)。

新しいリポジトリに切り出すには:

```bash
# OpenFDTD の作業ツリーで
git subtree split -P OpenFEM -b openfem-main

# 空の新規リポジトリ (例 Sirokujira/OpenFEM) を作ってから
git push git@github.com:Sirokujira/OpenFEM.git openfem-main:main
```

切り出し後は `OpenFEM/` を OpenFDTD 側から削除してください。
